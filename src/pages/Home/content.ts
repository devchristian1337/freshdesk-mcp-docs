...
export type HomeContent = {
  title: string;
  subtitle: string;
  /* Etichette delle bande della superficie catalogo. Corte e in mono:
     sono divisori, non titoli di sezione. */
...
export const homeContent: Record<Locale, HomeContent> = {
  en: {
    bandReference: 'Reference', bandFeatures: 'Capabilities', bandRunning: 'Running it',
    title: 'Bring Freshdesk into your AI assistant', subtitle: 'freshdesk-mcp exposes 64 tools and 2 prompts through Model Context Protocol: tickets, conversations, contacts, companies, knowledge base, and custom fields in natural language.', start: 'Get started', configureClient: 'Configure client', localMode: 'Local · stdio', remoteMode: 'Remote · HTTP', localText: 'The client starts the server locally in Claude Desktop, Cursor, or Windsurf. Credentials stay in environment variables.', remoteText: 'A Docker or Railway deployment serves /mcp, with a health check on /health. Credentials travel per request through headers.', seeConfiguration: 'See configuration', seeInstallation: 'See installation', ctaTitle: 'Ready to get started?', ctaText: 'You only need a Freshdesk API key and an MCP client. It takes a few minutes.', installServer: 'Install server', githubCode: 'Code on GitHub', request: 'request', tool: 'tool', response: 'response', requestExample: '“Show ticket #12345 and its full conversation”', ticketSubject: 'Login error', active: 'active', registry: commonRegistry.map((label) => ({label, promptLabel: 'Guided prompts create_ticket and create_reply'})),
    features: [
  ...
  es: {
    bandReference: 'Referencia', bandFeatures: 'Características', bandRunning: 'Ejecución',
